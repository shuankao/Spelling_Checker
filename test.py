...
def candidates(word): 
    ...
    return (known([word]) or (known(first_edit)) or (known(edits2())) or (known(edits3(second_edits))) or [word])
...
option = st.selectbox('Choosing a word...',(WORDS.keys()))
...
if text == corrected_text:
    txt = st.success('{} is the correct spelling!'.format(text))
else:
    txt = st.error('Correction: {}'.format(corrected_text))
